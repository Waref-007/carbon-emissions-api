...
def combine_engine_results(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "errors_preview": combined_errors[:50],
...
def run_engine_for_uploaded_sources(uploaded_sources: List[Dict[str, Any]], parser_options: Dict[str, Any]) -> Dict[str, Any]:
...
def write_summary_sheet(writer, result: dict):
    ...
    row_i = 7
    for item in takeaways or ["No takeaways available."]:
        ws[f"D{row_i}"] = item
        row_i += 1
    ...
    if insights:
        for item in insights:
            ws[f"G{row_i}"] = item.get("title", "")
            ws[f"H{row_i}"] = item.get("body", "")
            row_i += 1
    else:
...
def build_excel_report(result: dict, uploaded_files_info: list) -> io.BytesIO:
    ...
    dfs = {
        ...
        "Executive Summary Data": build_executive_summary_dataframe(result),
        ...
        "Confidence": pd.DataFrame([result.get("confidence_score", {})]) if result.get("confidence_score") else pd.DataFrame(),
        "Data Quality": pd.DataFrame([result.get("data_quality", {})]) if result.get("data_quality") else pd.DataFrame(),
        "Parser Diagnostics": pd.DataFrame(result.get("parser_diagnostics", [])),
        "Parser Options": pd.DataFrame([result.get("parser_options", {})]) if result.get("parser_options") else pd.DataFrame(),
